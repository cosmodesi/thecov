...
class SurveyWindow:
    ...
    def save_cartesian_ffts(self, filename):
        np.savez(filename if filename.strip()[-4:] == '.npz' else f'{filename}.npz', **{f'W{k.replace("W","")}': self._W[k] for k in self._W.keys()}, **{
                 f'I{k.replace("I","")}': self.I[k] for k in self.I.keys()}, BoxSize=self.BoxSize, Nmesh=self.Nmesh, alpha=self.alpha)
...
class GaussianSurveyWindowCovariance(PowerMultipoleCovariance):
    ...
    def compute_covariance(self):
        self._Pfit = {key: self._P[key](self.kmid) for key in self._P.keys()}

        Cov = np.array([self._compute_gaussian_covariance_row(ki)
                       for ki, k in enumerate(self.kmid)])
        ...
        C = {
            k: self._multipole_covariance[k].cov for k in self._multipole_covariance}
